...
"== eyeball test for ray trace (100 samples): lambertian material and
two spheres"
...
;; example tests
(define test-ray (make-ray fl3-zero (Float3 0.0 0.0 -1.0)))
(check-within (map hit->info (hit-test sphere-1 test-ray 0.0))
              (list (Hit-Info 'IN 1.5 (Float3 0.0 0.0 -1.5)
                              (Float3 0.0 0.0 1.0))
                    (Hit-Info 'OUT 2.5 (Float3 0.0 0.0 -2.5)
                              (Float3 0.0 0.0 -1.0))) 0.001)
(define hit1 (Hit 'IN 2.1 (Float3 -1.2 0.7 -0.9) (Float3 0.8 -0.6 1.1)
                  flat-black))
(define hit2 (Hit 'OUT 2.0 (Float3 1.3 0.6 -0.1) (Float3 -1.8 -1.6 0.1)
                  normal-material))

(check-expect (entry? hit1) #t)
(check-expect (entry? hit2) #f)
(check-expect (hit< hit1 hit2) #f)
(check-expect (hit< hit2 hit1) #t)

(check-within (map hit->info (hit-test (make-sphere (Float3 0.0 0.0 -3.0)
                                                    1.0 flat-black)
                                       (make-ray fl3-zero
                                                 (Float3 0.0 0.0 -1.0))
                                       0.0))
              (list
               (Hit-Info 'IN 2.0 (Float3 0.0 0.0 -2.0) (Float3 0.0 0.0 1.0))
               (Hit-Info 'OUT 4.0 (Float3 0.0 0.0 -4.0) (Float3 0.0 0.0 -1.0)))
              0.05)
