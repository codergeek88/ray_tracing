...
(: coord->rgb-coord : Float -> Float)
;; convert a coordinate of a vector to an RGB coordinate between 0 and 1
(define (coord->rgb-coord c)
  (* 0.5 (+ 1.0 c)))

(: map-norm->rgb : Float3 -> RGB)
;; maps a ray to RGB
(define (map-norm->rgb norm)
  (match norm
    [(Float3 x y z) (RGB (coord->rgb-coord x) (coord->rgb-coord y)
                         (coord->rgb-coord z))]))
...
(define normal-material
  (Material
   (lambda ([ray : Ray] [pt : Float3] [norm : Float3])
     (make-reflect-info (map-norm->rgb norm) ray))))
