...
;; Kishan Patel
;;
;; This module defines the representation the graphical objects
;; in the scene
;;
...
(define (list->object os)
  (Object
   (lambda ([ray : Ray] [min-t : Float])
     (local
       {(: acc-hit-list : Object Hit-List -> Hit-List)
        ;; helper function which adds the hit-list of an object
        (define (acc-hit-list obj acc)
          (merge-hit-lists (hit-test obj ray min-t) acc))}
       (foldl acc-hit-list '() os)))))
