...
;; of the ray w.r.t. the combined object.
;; For example, if we are inside the second
;; object, then an entry hit for the first object can be discarded.
;; We determine the status of the ray based on the parity of the next hit
...
(define (object-intersect obj1 obj2)
  (Object
   (lambda ([ray : Ray] [min-t : Float])
     (intersect-hit-lists (hit-test obj1 ray min-t)
                          (hit-test obj2 ray min-t)))))

(: subtract-hit-lists : Hit-List Hit-List -> Hit-List)
;; given two hit lists corresponding to two objects, compute the hit list for
;; the set subtraction of the two objects. 
