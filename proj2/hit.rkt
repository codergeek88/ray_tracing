...
;; get emission from a hit
(define (get-emission ray hit)
  (match hit
    [(Hit _ _ _ norm material)
     (match material
       [(Material emit scatter)
        (emit ray norm)])]))
...
(define (first-entry hits)
  (match hits
    [(cons hit hitr)
     (if (and (entry? hit) (< (Hit-t hit) (- +inf.0 1.0)))
         (Some hit) (first-entry hitr))]
    ['() 'None]))
...
;; extract the hit-info from a Hit record; we also convert
;; infinity to fl-max so that check-within will work
(define (hit->info hit)
  (match hit
    [(Hit p t pt n _)
     (if (= t +inf.0)
         (Hit-Info p fl-max pt n)
         (Hit-Info p t pt n))]))
